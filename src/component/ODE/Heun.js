...
class Heun extends Component {
  constructor(props){
    super(props);
    this.state={submitted:false,y:"",h:"",e:"",start:"",end:"",showOutput:false};
    X=[];
    Y=[];
    XE=[];
    YE=[];
  ...
  Heun(event){
    ...
    for(let i=start;i<end;i+=h){
      //nextY=nextY+((func(nextX,nextY)+func(nextX+=h,euler(nextX,nextY,h)))/2)*h;
      let s0=func(nextX,nextY);
      let y1=euler(nextX,nextY,h);
      nextX=nextX+h;
      let s1=func(nextX,y1);
      let sa=(s0+s1)/2;
      nextY=nextY+sa*h;
      XE.push(nextX);
      YE.push(nextY);
